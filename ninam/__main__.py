...
def assert_wrong_bitsize(bitsize=2):
    ...
    if 2**bitsize > len(WHITESPACE):
        raise Exception(
            f"Not enough whitespace symbols to use {bitsize} per whitespace "
        )
...
def encode(text: str, payload: str, bitsize=2):
    ...
    if len(payload) > max_size:
        logging.warning(
            f"Your payload ({len(payload)} bytes) exeed the amount of white space available in the text ({max_size} bytes). Spaces will be added at the end of the file"
        )
...
if __name__ == "__main__":
    ...
    encode_parser.add_argument(
        "-p", "--payload", required=True, help="Payload to encode"
    )
    encode_parser.add_argument(
        "-b",
        "--bitsize",
        ...
        type=int,
        default=2,
        help="How many bit per white space",
    )
